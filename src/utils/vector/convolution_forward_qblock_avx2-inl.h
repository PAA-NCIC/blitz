...
for (size_t bk = 0; bk < lk / (KREG * VEC_LEN); ++bk) {
  ...
  for (size_t bpq = 0; bpq < lpq / PQREG; ++bpq) {
    ...
    for (size_t bc = 0; bc < lc; ++bc) {
      // PQREG = 6
      // KREG = 2
      Fvec[0] = _mm256_load_ps(F_pack + bk * (CBLOCK * KREG * VEC_LEN) + bc * (KREG * VEC_LEN));
      Fvec[1] = _mm256_load_ps(F_pack + bk * (CBLOCK * KREG * VEC_LEN) + bc * (KREG * VEC_LEN) + VEC_LEN);
      Ivec[0] = _mm256_set1_ps(I_pack[bpq * PQREG * CBLOCK + bc]);
      Ivec[1] = _mm256_set1_ps(I_pack[bpq * PQREG * CBLOCK + CBLOCK + bc]);
      _mm_prefetch((char*)(F_pack + bk * (CBLOCK * KREG * VEC_LEN) + (bc + 1) * (KREG * VEC_LEN) + KREG * VEC_LEN), _MM_HINT_T0);
      ...
      Ivec[0] = _mm256_set1_ps(*(I_pack + bpq * PQREG * CBLOCK + CBLOCK * 2 + bc));
      Ivec[1] = _mm256_set1_ps(*(I_pack + bpq * PQREG * CBLOCK + CBLOCK * 3 + bc));
      ...
      Ivec[0] = _mm256_set1_ps(*(I_pack + bpq * PQREG * CBLOCK + CBLOCK * 4 + bc));
      Ivec[1] = _mm256_set1_ps(*(I_pack + bpq * PQREG * CBLOCK + CBLOCK * 5 + bc));
