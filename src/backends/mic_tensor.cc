...
MICTensor<DType>::~MICTensor() {
  free(this->data_);
}

template<typename DType>
inline void MICTensor<DType>::Fill(DType value) {
...
template<typename DType>
inline DType* MICTensor<DType>::Slice(size_t index) {
  return this->data_ + index;
}

template<typename DType>
inline const DType* MICTensor<DType>::Slice(size_t index) const {
  return this->data_ + index;
}

template<typename DType>
inline void MICTensor<DType>::Allocate() {
  size_t size = this->shape_.size();
  // TODO(keren) memkind
...
template<typename DType>
inline void MICTensor<DType>::OutputCSV(ofstream* ofs) const {
