...
int main(int argc, char** argv) {
  ...
  // create descriptors
  createTensor4dDesc<float>(&input_desc);
  createTensor4dDesc<float>(&output_desc);
  createFilterDesc<float>(&filter_desc);
  createConvolution2DDesc<float>(&conv_desc);
  // set descriptors
  setTensor4dNHWCDesc<float>(&input_desc, N, C, H, W);
  setTensor4dNHWCDesc<float>(&output_desc, N, K, P, Q);
  setFilterDesc<float>(&filter_desc, K, C, R, S);
  setConvolution2DDesc<float>(&conv_desc, pad_h, pad_w, str_h, str_w);
  // run convolution
  if (phase == "forward") {
